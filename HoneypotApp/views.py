...
def DownloadFileDataRequest(request):
    if request.method == 'POST':
        ...
        con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'HoneypotAppDB',charset='utf8')
        with con:
            cur = con.cursor()
...
def DownloadFile(request):
    if request.method == 'GET':
        ...
        #type="text"
        output+='<tr><td><font size="" color="black">Password</b></td><td><input type="password" name="t2">'
        context= {'data':output}
...
def UploadFile(request):
    if request.method == 'POST':
        ...
        db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'HoneypotAppDB',charset='utf8')
        db_cursor = db_connection.cursor()
        student_sql_query = "INSERT INTO files(username,filename,filepassword,shares) VALUES('"+user+"','"+name+"','"+password+"','"+shares+"')"
        
        #data=files(username,password,contact,email,address)
        #data.save()
...
def Signup(request):
    if request.method == 'POST':
      ...
      #data=Register(username,password,contact,email,address)
      #data.save()
